...
if [ $INSTALLED_RELEASE -lt 2 ]; then
    curl -sfLO https://github.com/hm-riscv/vscode-riscv-venus/releases/download/v1.7.0/riscv-venus-1.7.0.vsix
    code-server --install-extension ./riscv-venus-1.7.0.vsix
    code-server --install-extension /opt/extensions/zhwu95.riscv-0.0.8.vsix
    rm riscv-venus-1.7.0.vsix
fi
